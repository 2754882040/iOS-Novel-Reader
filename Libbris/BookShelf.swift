...
struct BookShelf: View {
    init(){
        _datas = StateObject(wrappedValue: DownloadJson(url:"http://libbris2021.us-west-2.elasticbeanstalk.com/ws/book/category/11?start=1&size=9"))
    ...
    var body: some View {
        NavigationView{
            ...
            ScrollView(.vertical){
                if(books.count == 0){Text("No Books").frame(alignment:.center)}
                else{
                    ...
                    LazyVGrid(columns: columns){
                        ForEach(items,id: \.self){item in
                            ZStack{
                                if(item % 3 == 0){
                                    Image("bookshelf").offset(x: /*@START_MENU_TOKEN@*/10.0/*@END_MENU_TOKEN@*/, y: 110)}
